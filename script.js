function showFloatingPoints(event, text) {
    const floatingText = document.createElement('div');
    floatingText.className = 'points-increment';
    floatingText.textContent = "🍪 +" + text;
  
    document.body.appendChild(floatingText);
  
    // Sijoita kelluva teksti klikkauksen kohdalle
    floatingText.style.position = 'absolute';
    floatingText.style.left = `${event.clientX}px`;
    floatingText.style.top = `${event.clientY - 30}px`;
  ...
  class KeksiKlikkeri {
    constructor() {
      ...
      this.keksiNappi.addEventListener('click', (event) => {
        this.klikkaaKeksia();
        const bonus = this.paivitykset.find(p => p.tyyppi === 'klikkausBonus')?.maara || 0;
        showFloatingPoints(event, this.klikkausteho + bonus);
      });
